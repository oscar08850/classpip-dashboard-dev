...
import {
  ...
  AlumnoJuegoDeCompeticionTorneo,
  EquipoJuegoDeCompeticionTorneo,
  Cuestionario,
  JuegoDeAvatar,
  FamiliaAvatares,
  AlumnoJuegoDeAvatar,
  ...
  AlumnoJuegoDeCuestionarioSatisfaccion,
  Rubrica,
  JuegoDeControlDeTrabajoEnEquipo,
  AlumnoJuegoDeControlDeTrabajoEnEquipo,
...
import { AlumnoJuegoDeLingo } from "src/app/clases/AlumnoJuegoDeLingo";
...
export class JuegoComponent implements OnInit {
  ...
  seleccionModoPresentacion: string[] = [
  ...
  seleccionModoPresentacionKahoot: string[] = [
    "Mostrar pregunta",
    "No mostrar pregunta",
  ];
  ...
  ngOnInit() {
    ...
    // ir a buscarla
    this.peticionesAPI
      .DameEquiposDelGrupo(this.grupo.id)
      .subscribe((equipos) => {
        if (equipos[0] !== undefined) {
          ...
          for (const equipo of equipos) {
            this.peticionesAPI
              .DameAlumnosEquipo(equipo.id)
              .subscribe((alumnos: Alumno[]) => {
                this.relacionAlumnosEquipos.push({
                  equipoId: equipo.id,
                  alumnos,
                });
                console.log(
                  "relacion alumnos equipos",
                  this.relacionAlumnosEquipos
                );
              });
          }
        } else {
    ...
    this.peticionesAPI
      .DameRubricasProfesor(this.profesorId)
      .subscribe((rubricas) => {
  ...
  ModoDeJuegoSeleccionado(modo: ChipColor) {
    ...
    if (
      this.tipoDeJuegoSeleccionado === "Juego De Avatar" &&
      this.modoDeJuegoSeleccionado === "Equipos"
    ) {
      Swal.fire(
        "Alerta",
        "Aún no es posible el juego de avatares en equipo",
        "warning"
      );
    } else if (
      this.tipoDeJuegoSeleccionado === "Juego De Cuentos" &&
      this.modoDeJuegoSeleccionado === "Equipos"
    ) {
      Swal.fire(
        "Alerta",
        "Aún no es posible el juego de libros en equipo",
        "warning"
      );
    } else if (
      this.tipoDeJuegoSeleccionado === "Juego De Geocaching" &&
      this.modoDeJuegoSeleccionado === "Equipos"
    ) {
      Swal.fire(
        "Alerta",
        "Aún no es posible el juego de geocaching en equipo",
        "warning"
      );
      ...
      Swal.fire(
        "Alerta",
        "No existe el juego de cuestionario de satisfacción en equipo",
        "warning"
      );
    } else {
  ...
  RecibeRubricaElegida($event) {
    ...
    this.rubricaElegida.Criterios.forEach(() => {
      this.parentPesos.push(
        this.PesoPorDefecto(this.rubricaElegida.Criterios.length)
      );
    });
  }
    ...
    if (
      ...
      for (let i = 0; i < evaluados.length; i++) {
        if (!fill) {
          ...
          console.log(
            "vamos a buscar evaluadores para el equipo " + evaluados[i]
          );
          const equipoEvaluado = this.relacionAlumnosEquipos.find(
            (equipo) => equipo.equipoId === evaluados[i]
          );
          ...
          while (cont < this.numeroDeMiembros) {
            ...
            if (
              equipoEvaluado.alumnos.some(
                (alumno) => alumno.id === evaluadoresDesordenados[j]
              )
            ) {
      ...
      for (let i = 0; i < evaluados.length; i++) {
        if (!fill) {
          ...
          if (
            ...
            for (const equipo of this.relacionAlumnosEquipos) {
              if (equipo.equipoId === evaluados[i]) {
                evaluadores = this.DameEvaluadores()
                  .filter(
                    ({ id: id1 }) =>
                      !equipo.alumnos.some(({ id: id2 }) => id1 === id2)
                  )
                  .map((item) => item.id);
              }
            }
            ...
            evaluadores = this.DameEvaluadores()
              .filter((item) => item.id !== evaluados[i])
              .map((item) => item.id);
          }
          evaluadores = this.Shuffle(evaluadores);
          if (
          ...
          this.relacionesMap.set(
            evaluados[i],
            evaluadores.filter((item) => !isNaN(item))
          );
        }
      }
    } while (
      this.ComprobarSiTodosTienenEvaluadores() === false &&
      fill === true
    );
    console.log("Relaciones object", this.relacionesMap);
  ...
  CriterioDeEvaluacionSeleccionado(criterioEvaluacion: ChipColor) {
    ...
    if (this.criterioEvaluacionSeleccionado === "Por pesos") {
      ...
      for (let i = 0; i < this.rubricaElegida.Criterios.length; i++) {
        ...
        for (
          let j = 0;
          j < this.rubricaElegida.Criterios[i].Elementos.length;
          j++
        ) {
          this.pesosArray[i].push(
            this.PesoPorDefecto(
              this.rubricaElegida.Criterios[i].Elementos.length
            )
          );
        }
      }
  ...
  PonPreguntaAbierta() {
    ...
    this.dataSourcePreguntasAbiertas = new MatTableDataSource(
  ...
  BorraPreguntaAbierta(preguntaAbierta) {
    this.listaPreguntasAbiertas = this.listaPreguntasAbiertas.filter(
      (pregunta) => pregunta !== preguntaAbierta
    );
    this.dataSourcePreguntasAbiertas = new MatTableDataSource(
      this.listaPreguntasAbiertas
    );
    ...
    if (this.modoDeJuegoSeleccionado === "Individual") {
      ...
      if (this.equiposEvaluacionSeleccionado === "Por Equipos") {
        ...
        for (let i = 0; i < this.relacionAlumnosEquipos.length; i++) {
          if (
            this.relacionAlumnosEquipos[i].alumnos.length > tamañoEquipoMayor
          ) {
  ...
  GuardarDescripcionEvaluacion() {
    this.descripcionJuegoEvaluacion =
      this.myForm.value.DescripcionJuegoEvaluacion;
  }
  ...
  PenalizacionChanged(name: string, value: string): void {
    ...
    if (tipo === "num") {
      ...
      this.penalizacionArray[criterio][elemento] = {
  ...
  CrearJuegoEvaluacion() {
    ...
    this.peticionesAPI.CrearJuegoDeEvaluacion(juego).subscribe((res) => {
      ...
      this.comService.EnviarNotificacionGrupo(
        1,
        this.grupo.id,
        `Nuevo Juego de Evaluación para el Grupo ${this.grupo.Nombre}: ${this.nombreDelJuego}`
      );
      ...
      this.relacionesMap.forEach((value: number[], key: number) => {
        if (
          this.modoDeJuegoSeleccionado === "Equipos" &&
          this.equiposEvaluacionSeleccionado === "Por Equipos"
        ) {
          ...
          this.peticionesAPI
            .CrearEquipoJuegoDeEvaluacion(equipo)
            .subscribe((equipores) =>
              console.log("EquipoJuegoEvaluado", equipores)
            );
        } else if (
          this.modoDeJuegoSeleccionado === "Equipos" &&
          this.equiposEvaluacionSeleccionado === "Individualmente"
          ...
          this.peticionesAPI
            .CrearAlumnoJuegoDeEvaluacion(alumno)
            .subscribe((alumnosres) =>
              console.log("AlumnoJuegoEvaluado", alumnosres)
            );
        }
      });
    ...
    this.peticionesAPI
      ...
      .subscribe((juegoCreado) => {
        ...
        this.comService.EnviarNotificacionGrupo(
          1,
          this.grupo.id,
          `Nuevo Juego de Puntos para el Grupo ${this.grupo.Nombre}: ${this.nombreDelJuego}`
        );
        ...
        this.puntosDelJuego.forEach((punto) =>
          this.peticionesAPI
            .AsignaPuntoJuego(
              new AsignacionPuntosJuego(punto.id, this.juego.id)
            )
            .subscribe()
        );
        ...
        if (this.nivelesDelJuego !== undefined) {
          this.nivelesDelJuego.forEach((nivel) =>
            this.peticionesAPI.CreaNivel(nivel, this.juego.id).subscribe()
          );
          // Guardamos los logos de los niveles
          this.logosNiveles.forEach((logo) =>
            this.peticionesAPI.PonImagenNivel(logo).subscribe()
          );
        }
        ...
        if (this.modoDeJuegoSeleccionado === "Individual") {
          ...
          for (let i = 0; i < this.alumnosGrupo.length; i++) {
            ...
            this.peticionesAPI
              .InscribeAlumnoJuegoDePuntos(
                new AlumnoJuegoDePuntos(this.alumnosGrupo[i].id, this.juego.id)
              )
              .subscribe();
          }
        } else {
          ...
          for (let i = 0; i < this.equiposGrupo.length; i++) {
            ...
            this.peticionesAPI
              .InscribeEquipoJuegoDePuntos(
                new EquipoJuegoDePuntos(this.equiposGrupo[i].id, this.juego.id)
              )
              .subscribe();
          }
        }
  ...
  RegistraDificultad() {
    ...
    if (this.familiaSeleccionada.relacion === true) {
      ...
      this.numerocartas = this.numerocartas;
    }
  ...
  async seleccionarcartas() {
    this.vectorcartas = await this.peticionesAPI
      .DameCartasFamilia(this.familiaSeleccionada.id)
      .toPromise();
  ...
  CartaSeleccionada(i) {
    ...
    console.log(
      "CARTAS SIN SELECCIONAR:",
      this.cartaseleccionada1,
      this.cartaseleccionada2
    );
  ...
  RegistraCartas() {
    console.log(
      "CARTAS QUE VAN A SER USADAS EN EL JUEGO:",
  ...
  CrearJuegoDeLingo() {
    ...
    this.peticionesAPI
      .CreaJuegoDeLingo(JuegoDeLingo, this.grupo.id)
      .subscribe((juegoCreado) => {
        this.juego = juegoCreado;
        ...
        this.comService.EnviarNotificacionGrupo(
          1,
          this.grupo.id,
          `Nuevo Juego de Lingo para el Grupo ${this.grupo.Nombre}: ${this.nombreDelJuego}`
        );
        ...
        if (this.modoDeJuegoSeleccionado === "Individual") {
          for (let i = 0; i < this.alumnosGrupo.length; i++) {
            this.peticionesAPI
              .InscribeAlumnoJuegoDeLingo(
                new AlumnoJuegoDeLingo(
                  false,
                  this.alumnosGrupo[i].id,
                  this.juego.id
                )
              ) //funciona JUEGO.ID?
              .subscribe();
          ...
          for (let i = 0; i < this.equiposGrupo.length; i++) {
            this.peticionesAPI
              .InscribeEquipoJuegoDeMemorama(
                new EquipoJuegoDeMemorama(
                  this.equiposGrupo[i].id,
                  this.juego.id
                )
              )
              .subscribe();
  ...
  CrearJuegoDeMemorama() {
    ...
    console.log(
      this.tipoDeJuegoSeleccionado,
      this.modoDeJuegoSeleccionado,
      this.familiaSeleccionada.id,
      true,
      this.nombreDelJuego,
      this.idcartas,
      this.puntuacionCorrecta,
      this.puntuacionIncorrecta,
      this.Mododificultad,
      this.TiempoDuracion
    );
    ...
    // tslint:disable-next-line:max-line-length
    JuegoMemoramaaentrtrar = new JuegoMEMORAMA(
      this.tipoDeJuegoSeleccionado,
      this.modoDeJuegoSeleccionado,
      this.familiaSeleccionada.id,
      true,
    ...
    this.peticionesAPI
      .CreaJuegoDeMemorama(JuegoMemoramaaentrtrar, this.grupo.id)
      .subscribe((juegoCreado) => {
        this.juego = juegoCreado;
        console.log(juegoCreado);
        console.log("Juego creado correctamente");
        ...
        this.comService.EnviarNotificacionGrupo(
          1,
          this.grupo.id,
          `Nuevo Juego de Memorama para el Grupo ${this.grupo.Nombre}: ${this.nombreDelJuego}`
        );
        ...
        if (this.modoDeJuegoSeleccionado === "Individual") {
          for (let i = 0; i < this.alumnosGrupo.length; i++) {
            this.peticionesAPI
              .InscribeAlumnoJuegoDeMemorama(
                new AlumnoJuegoDeMemorama(
                  this.alumnosGrupo[i].id,
                  this.juego.id,
                  0,
                  "00:00"
  ...
  CrearJuegoDeColeccion() {
    this.peticionesAPI
      .CreaJuegoDeColeccion(
        new Juego(
          this.tipoDeJuegoSeleccionado,
          this.modoDeJuegoSeleccionado,
          this.modoAsignacion,
          this.coleccionSeleccionada.id,
          undefined,
          undefined,
          undefined,
          undefined,
      ...
      .subscribe((juegoCreado) => {
        ...
        this.comService.EnviarNotificacionGrupo(
          1,
          this.grupo.id,
          `Nuevo Juego de Colección para el Grupo ${this.grupo.Nombre}: ${this.nombreDelJuego}`
        );
        ...
        if (this.modoDeJuegoSeleccionado === "Individual") {
          ...
          for (let i = 0; i < this.alumnosGrupo.length; i++) {
            this.peticionesAPI
              .InscribeAlumnoJuegoDeColeccion(
                new AlumnoJuegoDeColeccion(
                  this.alumnosGrupo[i].id,
                  this.juego.id
                )
              )
          ...
          for (let i = 0; i < this.equiposGrupo.length; i++) {
            this.peticionesAPI
              .InscribeEquipoJuegoDeColeccion(
                new EquipoJuegoDeColeccion(
                  this.equiposGrupo[i].id,
                  this.juego.id
                )
              )
  ...
  TengoDatosLingo() {
    if (
      this.myForm.value.PalabraSecreta === "" ||
      this.myForm.value.NumeroDeIntentos === ""
    ) {
      return false;
    } else {
      return true;
  ...
  TengoPuntuaciones() {
    if (
      this.myForm.value.PuntuacionCorrecta === "" ||
      this.myForm.value.PuntuacionIncorrecta === ""
    ) {
      return false;
    } else {
      return true;
  ...
  ModalidadDeJuegoSeleccionada(modalidad: ChipColor) {
    if (
      this.modoDeJuegoSeleccionado === "Equipos" &&
      modalidad.nombre === "Kahoot"
    ) {
      Swal.fire(
        "Atención",
        "La modalidad Kahoot no está implementada para los juegos de cuestionario en equipo",
        "error"
      );
    } else {
  ...
  CrearJuegoDeCuestionario() {
    ...
    // tslint:disable-next-line:max-line-length
    const juego = new JuegoDeCuestionario(
      this.nombreDelJuego,
      this.tipoDeJuegoSeleccionado,
      this.modoDeJuegoSeleccionado,
      ...
      this.puntuacionCorrecta,
      this.puntuacionIncorrecta,
      this.modoPresentacion,
      false,
      false,
      this.profesorId,
      this.grupo.id,
      this.cuestionario.id,
      this.tiempoLimite
    );
    console.log("voy a crear juego ", juego);
    this.peticionesAPI
      .CreaJuegoDeCuestionario(juego, this.grupo.id)
      .subscribe((juegoCreado) => {
        ...
        this.comService.EnviarNotificacionGrupo(
          1,
          this.grupo.id,
          `Nuevo Juego de Cuestionario para el Grupo ${this.grupo.Nombre}: ${this.nombreDelJuego}`
        );

        // tslint:disable-next-line:max-line-length
        if (
          this.modoDeJuegoSeleccionado === "Individual" ||
          (this.modoDeJuegoSeleccionado === "Equipos" &&
            this.modoPresentacion !== "Primero")
        ) {
          ...
          for (let i = 0; i < this.alumnosGrupo.length; i++) {
            // tslint:disable-next-line:max-line-length
            this.peticionesAPI
              .InscribeAlumnoJuegoDeCuestionario(
                new AlumnoJuegoDeCuestionario(
                  0,
                  false,
                  this.juegoDeCuestionario.id,
                  this.alumnosGrupo[i].id
          ...
          for (let i = 0; i < this.equiposGrupo.length; i++) {
            // tslint:disable-next-line:max-line-length
            this.peticionesAPI
              .InscribeEquipoJuegoDeCuestionario(
                new EquipoJuegoDeCuestionario(
                  0,
                  false,
                  this.juegoDeCuestionario.id,
                  this.equiposGrupo[i].id
  ...
  CrearJuegoDeAvatar() {
    ...
    juego.CriteriosPrivilegioComplemento1 =
      this.myForm.value.criterioPrivilegioComplemento1;
    juego.CriteriosPrivilegioComplemento2 =
      this.myForm.value.criterioPrivilegioComplemento2;
    juego.CriteriosPrivilegioComplemento3 =
      this.myForm.value.criterioPrivilegioComplemento3;
    juego.CriteriosPrivilegioComplemento4 =
      this.myForm.value.criterioPrivilegioComplemento4;
    juego.CriteriosPrivilegioVoz = this.myForm.value.criterioPrivilegioVoz;
    juego.CriteriosPrivilegioVerTodos =
      this.myForm.value.criterioPrivilegioVerTodos;

    this.peticionesAPI
      .CreaJuegoDeAvatar(juego, this.grupo.id)
      .subscribe((nuevoJuego) => {
        ...
        this.comService.EnviarNotificacionGrupo(
          1,
          this.grupo.id,
          `Nuevo Juego de Avatar para el Grupo ${this.grupo.Nombre}: ${this.juegoDeAvatar.NombreJuego}`
        );
        ...
        if (this.modoDeJuegoSeleccionado === "Individual") {
          ...
          if (this.modoDeJuegoSeleccionado === "Individual") {
            ...
            for (let i = 0; i < this.alumnosGrupo.length; i++) {
              ...
              console.log("inscribo");
              this.peticionesAPI
                .InscribeAlumnoJuegoDeAvatar(
                  new AlumnoJuegoDeAvatar(
                    this.alumnosGrupo[i].id,
                    this.juegoDeAvatar.id
                  )
                )
  ...
  GuardarNumeroDeJornadas() {
    ...
    if (this.numeroDeJornadas === undefined || isNaN(this.numeroDeJornadas)) {
      ...
      Swal.fire(
        "Introduzca un número de jornadas válido",
        "Le recordamos que debe ser un número",
        "error"
      );
    } else {
  ...
  AnadirPuntos() {
    ...
    if (!isNaN(this.nuevaPuntuacion)) {
      ...
      Swal.fire(
        "Introduzca una puntuación válida",
        "Le recordamos que debe ser un Número",
        "error"
      );
    }
  ...
  AnadirFila() {
    ...
    if (i < NumeroParticipantes) {
      ...
      Swal.fire(
        "No es posible añadir otra fila",
        "Ya puntuan todos los participantes",
        "error"
      );
    }
  }
  ...
  EliminarFila() {
    ...
    if (i > 1) {
      ...
      Swal.fire(
        "No es posible eliminar otra fila",
        "Como mínimo debe puntuar un participante",
        "error"
      );
    }
  }
    ...
    this.peticionesAPI
      ...
      .subscribe((juegoCreado) => {
        ...
        this.comService.EnviarNotificacionGrupo(
          1,
          this.grupo.id,
          `Nuevo Juego De Competición Liga para el Grupo ${this.grupo.Nombre}: ${this.nombreDelJuego}`
        );

        // Creamos las jornadas
        console.log("voy a crear jornadas");
        this.calculos
          .CrearJornadasLiga(this.numeroDeJornadas, this.juego.id)
          .subscribe((jornadas) => {
            ...
            if (this.modoDeJuegoSeleccionado === "Individual") {
              // tslint:disable-next-line:max-line-length
              this.calculos.calcularLiga(
                this.alumnosGrupo.length,
                this.jornadasLiga.length,
                this.alumnosGrupo,
                this.grupo.id,
                this.jornadasLiga
              );
              ...
              for (let i = 0; i < this.alumnosGrupo.length; i++) {
                // tslint:disable-next-line:max-line-length
                this.peticionesAPI
                  .InscribeAlumnoJuegoDeCompeticionLiga(
                    new AlumnoJuegoDeCompeticionLiga(
                      this.alumnosGrupo[i].id,
                      this.juego.id
                    )
                  )
              ...
              for (let i = 0; i < this.equiposGrupo.length; i++) {
                // tslint:disable-next-line:max-line-length
                this.peticionesAPI
                  .InscribeEquipoJuegoDeCompeticionLiga(
                    new EquipoJuegoDeCompeticionLiga(
                      this.equiposGrupo[i].id,
                      this.juego.id
                    )
                  )
            ...
            Swal.fire(
              "Juego de competición tipo liga creado correctamente",
              " ",
              "success"
            );
            // El juego se ha creado como activo. Lo añadimos a la lista correspondiente
  ...
  CrearJuegoDeCompeticionFormulaUno() {
    ...
    this.peticionesAPI
      .CreaJuegoDeCompeticionFormulaUno(
        new Juego(
          this.tipoDeJuegoSeleccionado +
            " " +
            this.tipoDeCompeticionSeleccionado,
      ...
      .subscribe((juegoCreado) => {
        ...
        this.comService.EnviarNotificacionGrupo(
          1,
          this.grupo.id,
          `Nuevo Juego De Competición Fórmula Uno para el Grupo ${this.grupo.Nombre}: ${this.nombreDelJuego}`
        );

        this.calculos
          .CrearJornadasFormulaUno(this.numeroDeJornadas, this.juego.id)
          .subscribe((jornadas) => {
            this.jornadasFormulaUno = jornadas;
            this.sesion.TomaDatosJornadasJuegoComponent(
              this.jornadasFormulaUno
            );
            ...
            if (this.modoDeJuegoSeleccionado === "Individual") {
              ...
              for (let i = 0; i < this.alumnosGrupo.length; i++) {
                // tslint:disable-next-line:max-line-length
                this.peticionesAPI
                  .InscribeAlumnoJuegoDeCompeticionFormulaUno(
                    new AlumnoJuegoDeCompeticionFormulaUno(
                      this.alumnosGrupo[i].id,
                      this.juego.id
                    )
                  )
              ...
              for (let i = 0; i < this.equiposGrupo.length; i++) {
                // tslint:disable-next-line:max-line-length
                this.peticionesAPI
                  .InscribeEquipoJuegoDeCompeticionFormulaUno(
                    new EquipoJuegoDeCompeticionFormulaUno(
                      this.equiposGrupo[i].id,
                      this.juego.id
                    )
                  )
            ...
            Swal.fire(
              "Juego de competición tipo fórmula uno creado correctamente",
              " ",
              "success"
            );
  ...
  CrearCuadroTorneo() {
    ...
    if (this.modoDeJuegoSeleccionado === "Individual") {
      this.ParticipantesTorneo = this.calculos.calcularCuadro(
        this.alumnosGrupo
      );
    } else {
      this.ParticipantesTorneo = this.calculos.calcularCuadro(
  ...
  HayEnfrentamientoEntreFantasmas(): boolean {
    for (let i = 0; i < this.ParticipantesTorneo.length; i = i + 2) {
      if (
        this.ParticipantesTorneo[i] === undefined &&
        this.ParticipantesTorneo[i + 1] === undefined
      ) {
        return true;
      }
    }
  ...
  CrearJuegoDeCompeticionTorneo() {
    ...
    const nuevojuego = new Juego(
      this.tipoDeJuegoSeleccionado + " " + this.tipoDeCompeticionSeleccionado,
      this.modoDeJuegoSeleccionado,
      undefined,
      undefined,
      true,
    ...
    this.peticionesAPI
      .CreaJuegoDeCompeticionTorneo(nuevojuego, this.grupo.id)
      .subscribe((juegoCreado) => {
        this.juego = juegoCreado;
        this.sesion.TomaJuego(this.juego);
        this.juegoCreado = true;
        const evento: Evento = new Evento(
          ...
          this.juego.id,
          this.nombreDelJuego,
          "Juego De Competición Torneo"
        );
        this.calculos.RegistrarEvento(evento);
        this.comService.EnviarNotificacionGrupo(
          1,
          this.grupo.id,
          `Nuevo Juego De Competición Torneo para el Grupo ${this.grupo.Nombre}: ${this.nombreDelJuego}`
        );
        // Creamos las jornadas
        console.log("voy a crear jornadas");
        this.calculos
          .CrearJornadasTorneo(
            this.ParticipantesTorneo,
            this.juego.id,
            this.juego.ModeloTorneo
          )
          .subscribe((jornadas) => {
            this.jornadasTorneo = jornadas;
            console.log("Jornadas creadas correctamente");
            // Creamos Enfrentamientos
            this.calculos.CrearEnfrentamientosTorneo(
              this.ParticipantesTorneo,
              this.jornadasTorneo
            );
            ...
            if (this.modoDeJuegoSeleccionado === "Individual") {
              ...
              for (let i = 0; i < this.alumnosGrupo.length; i++) {
                ...
                if (this.alumnosGrupo[i] !== undefined) {
                  this.peticionesAPI
                    .InscribeAlumnoJuegoDeCompeticionTorneo(
                      new AlumnoJuegoDeCompeticionTorneo(
                        this.alumnosGrupo[i].id,
                        this.juego.id
                      )
                    )
              ...
              for (let i = 0; i < this.equiposGrupo.length; i++) {
                ...
                if (this.equiposGrupo[i] !== undefined) {
                  ...
                  this.peticionesAPI
                    .InscribeEquipoJuegoDeCompeticionTorneo(
                      new EquipoJuegoDeCompeticionTorneo(
                        this.equiposGrupo[i].id,
                        this.juego.id
                      )
                    )
            ...
            Swal.fire(
              "Juego de competición tipo Torneo creado correctamente",
              " ",
              "success"
            );
            // El juego se ha creado como activo. Lo añadimos a la lista correspondiente
  ...
  DamePuntosGeolocalizablesDelEscenario(escenario: Escenario) {
    console.log(
      "voy a mostrar los puntosgeolocalizables del escenario " + escenario.id
    );
    this.peticionesAPI
      .DamePuntosGeolocalizablesEscenario(escenario.id)
      .subscribe((res) => {
        if (res[0] !== undefined) {
          this.puntosgeolocalizablesEscenario = res;
          console.log(res);
          this.numeroDePuntosGeolocalizables =
            this.puntosgeolocalizablesEscenario.length;
          console.log(this.numeroDePuntosGeolocalizables);
          this.tengoEscenario = true;
        } else {
  ...
  TengoPuntuacionesGeocatching() {
    if (
      this.myForm.value.PuntuacionCorrectaGeo === "" ||
  ...
  GuardarPuntuacionGeocaching() {
    ...
    this.puntuacionCorrectaGeoBonus =
      this.myForm.value.PuntuacionCorrectaGeoBonus;
    this.puntuacionIncorrectaGeoBonus =
      this.myForm.value.PuntuacionIncorrectaGeoBonus;
  }
  ...
  CrearJuegoDeGeocaching() {
    // tslint:disable-next-line:max-line-length
    this.peticionesAPI
      .CreaJuegoDeGeocaching(
        new JuegoDeGeocaching(
          this.nombreDelJuego,
          this.tipoDeJuegoSeleccionado,
          this.puntuacionCorrectaGeo,
          this.puntuacionIncorrectaGeo,
          this.puntuacionCorrectaGeoBonus,
          this.puntuacionIncorrectaGeoBonus,
      ...
      .subscribe((juegoCreado) => {
        ...
        this.comService.EnviarNotificacionGrupo(
          1,
          this.grupo.id,
          `Nuevo Juego De Geocaching para el Grupo ${this.grupo.Nombre}: ${this.nombreDelJuego}`
        );
        ...
        for (let i = 0; i < this.alumnosGrupo.length; i++) {
          // tslint:disable-next-line:max-line-length
          this.peticionesAPI
            .InscribeAlumnoJuegoDeGeocaching(
              new AlumnoJuegoDeGeocaching(
                0,
                0,
                this.alumnosGrupo[i].id,
                this.juegoDeGeocaching.id
              )
            )
            .subscribe();
        }
  ...
  TipoDeVotacionSeleccionado(tipoVotacion: ChipColor) {
    ...
    if (
      this.modoDeJuegoSeleccionado === "Equipos" &&
      (tipoVotacion.nombre === "Todos A Uno" ||
        tipoVotacion.nombre === "Votar opciones")
    ) {
      Swal.fire(
        "Alerta",
        "Aún no es posible ete tipo de juego de votación en EQUIPO",
        "warning"
      );
    } else {
  ...
  GuardarQuienVota() {
    let radio = document.getElementsByName(
      "autovotacion"
  ...
  CrearJuegoDeVotacionUnoATodos() {
    ...
    this.peticionesAPI
      .CreaJuegoDeVotacionUnoATodos(juegoDeVotacion, this.grupo.id)
      .subscribe((juegoCreado) => {
        this.juego = juegoCreado;
        this.sesion.TomaJuego(this.juego);
        this.juegoCreado = true;
        ...
        this.comService.EnviarNotificacionGrupo(
          1,
          this.grupo.id,
          `Nuevo Juego De Votación Uno A Todos para el Grupo ${this.grupo.Nombre}: ${this.nombreDelJuego}`
        );
        ...
        if (this.modoDeJuegoSeleccionado === "Individual") {
          ...
          for (let i = 0; i < this.alumnosGrupo.length; i++) {
            // tslint:disable-next-line:max-line-length
            this.peticionesAPI
              .InscribeAlumnoJuegoDeVotacionUnoATodos(
                // tslint:disable-next-line:indent
                new AlumnoJuegoDeVotacionUnoATodos(
                  this.alumnosGrupo[i].id,
                  this.juego.id
                )
              )
          ...
          for (let i = 0; i < this.equiposGrupo.length; i++) {
            // tslint:disable-next-line:max-line-length
            this.peticionesAPI
              .InscribeEquipoJuegoDeVotacionUnoATodos(
                // tslint:disable-next-line:indent
                new EquipoJuegoDeVotacionUnoATodos(
                  this.equiposGrupo[i].id,
                  this.juego.id
                )
              )
        ...
        Swal.fire(
          "Juego de votación tipo Uno A Todos creado correctamente",
          " ",
          "success"
        );
  ...
  PonConcepto() {
    this.listaConceptos.push({
      nombre: this.myForm.value.NombreDelConcepto,
      peso: this.myForm.value.PesoDelConcepto,
    });
  ...
  CrearJuegoDeVotacionTodosAUno() {
    ...
    this.peticionesAPI
      .CreaJuegoDeVotacionTodosAUno(juegoDeVotacion, this.grupo.id)
      .subscribe((juegoCreado) => {
        this.juego = juegoCreado;
        this.sesion.TomaJuego(this.juego);
        this.juegoCreado = true;
        ...
        this.comService.EnviarNotificacionGrupo(
          1,
          this.grupo.id,
          `Nuevo Juego De Votación Todos A Uno para el Grupo ${this.grupo.Nombre}: ${this.nombreDelJuego}`
        );
        ...
        if (this.modoDeJuegoSeleccionado === "Individual") {
          ...
          for (let i = 0; i < this.alumnosGrupo.length; i++) {
            // tslint:disable-next-line:max-line-length
            this.peticionesAPI
              .InscribeAlumnoJuegoDeVotacionTodosAUno(
                new AlumnoJuegoDeVotacionTodosAUno(
                  this.alumnosGrupo[i].id,
                  this.juego.id
                )
              )
        ...
        Swal.fire(
          "Juego de votación tipo Todos A Uno creado correctamente",
          " ",
          "success"
        );
  ...
  CrearJuegoDeVotacionAOpciones() {
    ...
    console.log(juegoDeVotacion);
    this.peticionesAPI
      .CreaJuegoDeVotacionAOpciones(juegoDeVotacion, this.grupo.id)
      .subscribe((juegoCreado) => {
        this.juego = juegoCreado;
        this.sesion.TomaJuego(this.juego);
        this.juegoCreado = true;
        ...
        this.comService.EnviarNotificacionGrupo(
          1,
          this.grupo.id,
          `Nuevo Juego De Votación A Opciones para el Grupo ${this.grupo.Nombre}: ${this.nombreDelJuego}`
        );
        ...
        if (this.modoDeJuegoSeleccionado === "Individual") {
          ...
          for (let i = 0; i < this.alumnosGrupo.length; i++) {
            // tslint:disable-next-line:max-line-length
            this.peticionesAPI
              .InscribeAlumnoJuegoDeVotacionAOpciones(
                new AlumnoJuegoDeVotacionAOpciones(
                  this.alumnosGrupo[i].id,
                  this.juego.id
                )
              )
        ...
        Swal.fire(
          "Juego de votación A Opciones creado correctamente",
          " ",
          "success"
        );
  ...
  CrearJuegoDeCuestionarioDeSatisfaccion() {
    ...
    this.peticionesAPI
      .CreaJuegoDeCuestionarioSatisfaccion(
        juegoDeCuestionarioSatisfaccion,
        this.grupo.id
      )
      .subscribe((juegoCreado) => {
        ...
        this.comService.EnviarNotificacionGrupo(
          1,
          this.grupo.id,
          `Nuevo Juego De Cuestionario de Satisfacción para el Grupo ${this.grupo.Nombre}: ${this.nombreDelJuego}`
        );
        ...
        if (this.modoDeJuegoSeleccionado === "Individual") {
          ...
          for (let i = 0; i < this.alumnosGrupo.length; i++) {
            // tslint:disable-next-line:max-line-length
            this.peticionesAPI
              .InscribeAlumnoJuegoDeCuestionarioSatisfaccion(
                new AlumnoJuegoDeCuestionarioSatisfaccion(
                  false,
                  this.juego.id,
                  this.alumnosGrupo[i].id
                )
        ...
        Swal.fire(
          "Juego de cuestionario de satisfacción creado correctamente",
          " ",
          "success"
        );
  ...
  canExit(): Observable<boolean> {
    ...
    if (!this.creandoJuego) {
      ...
      const confirmacionObservable = new Observable<boolean>((obs) => {
        ...
        Swal.fire({
          title:
            "¿Seguro que quieres abandonar el proceso de creación del juego?",
  ...
  CrearJuegoCuento() {
    ...
    this.peticionesAPI
      .CrearJuegoCuento(juego, this.grupo.id)
      .subscribe((juego) => {
        ...
        if (this.modoDeJuegoSeleccionado === "Individual") {
          ...
          if (this.modoDeJuegoSeleccionado === "Individual") {
            ...
            for (let i = 0; i < this.alumnosGrupo.length; i++) {
              ...
              this.peticionesAPI
                .InscribeAlumnojuegoDeCuento(alumno, id)
                .subscribe(
                  (res) => {
                    console.log(res);
                  },
  ...
  crearRecursoJuegoCuento(idCuento: any) {
    this.peticionesAPI
      .crearRecursosJuegoCuento(idCuento, this.recursoParaCuento)
      .subscribe(
        (res) => {},
        (err) => {
          console.log(err);
  ...
  GuardarNumeroDeControles() {
    ...
    if (this.numeroDeControles === undefined || isNaN(this.numeroDeControles)) {
      ...
      Swal.fire(
        "Introduzca un número de controles válido",
        "Le recordamos que debe ser un número",
        "error"
      );
    } else {
  ...
  CrearJuegoDeControlDeTrabajoEnEquipo() {
    ...
    this.peticionesAPI
      .CreaJuegoDeControlDeTrabajoEnEquipo(
        juegoDeControlDeTrabajoEnEquipo,
        this.grupo.id
      )
      .subscribe((juegoCreado) => {
        ...
        this.comService.EnviarNotificacionGrupo(
          1,
          this.grupo.id,
          `Nuevo Juego de Control De Trabajo En Equipo para el Grupo ${this.grupo.Nombre}: ${this.nombreDelJuego}`
        );
        ...
        for (let i = 0; i < this.alumnosGrupo.length; i++) {
          // tslint:disable-next-line:max-line-length
          this.peticionesAPI
            .InscribeAlumnoJuegoDeControlDeTrabajoEnEquipo(
              // tslint:disable-next-line:indent
              new AlumnoJuegoDeControlDeTrabajoEnEquipo(
                this.alumnosGrupo[i].id,
                this.juego.id
              )
            )
        ...
        Swal.fire(
          "Juego de control de trabajo en equipo creado correctamente",
          " ",
          "success"
        );
