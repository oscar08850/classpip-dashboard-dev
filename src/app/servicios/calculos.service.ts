...
export class CalculosService {
  ...
  // REVISAR ESTO DE LOS RANKING PORQUE ME PARECE MUY COMPLICADO
  // NO SE POR QUE SE NECESITAN DOS FUNCIONES PARA RANKINS

  public DameRanking( listaAlumnosOrdenadaPorPuntos: any,
                      alumnosDelJuego: any,
                      nivelesDelJuego: any): any {

      const rankingJuegoDePuntosTotal: any[] = [];
      // tslint:disable-next-line:prefer-for-of
      for (let i = 0; i < listaAlumnosOrdenadaPorPuntos.length; i++) {
        let alumno: Alumno;
        let nivel: Nivel;
 ...
 */
            rankingJuegoDePuntosTotal[i] = new TablaAlumnoJuegoDePuntos (i + 1, alumno.Nombre, alumno.PrimerApellido,
            alumno.SegundoApellido, listaAlumnosOrdenadaPorPuntos[i].PuntosTotalesAlumno, nivel.Nombre);
        } else {
  /*         this.rankingJuegoDePuntos[i] = new TablaAlumnoJuegoDePuntos (i + 1, alumno.Nombre, alumno.PrimerApellido, alumno.SegundoApellido,
            this.listaAlumnosOrdenadaPorPuntos[i].PuntosTotalesAlumno);
 */
          rankingJuegoDePuntosTotal[i] = new TablaAlumnoJuegoDePuntos (i + 1, alumno.Nombre, alumno.PrimerApellido,
            alumno.SegundoApellido, listaAlumnosOrdenadaPorPuntos[i].PuntosTotalesAlumno);
        }
      }
      return rankingJuegoDePuntosTotal;
  }


  public DameRankingEquipos(
      ...
      for (let i = 0; i < listaEquiposOrdenadaPorPuntos.length; i ++) {
        ...
        .subscribe(historial => {
          ...
          if (i === listaEquiposOrdenadaPorPuntos.length - 1 ) {
            ...
            // tslint:disable-next-line:prefer-for-of
            for (let j = 0; i < rankingEquiposJuegoDePuntos.length - 1; j++) {
              rankingEquiposJuegoDePuntos[j].posicion = j + 1;
            }
  ...
  public DameRanking2(  listaAlumnosOrdenadaPorPuntos: any,
                        alumnosDelJuego: any,
                        nivelesDelJuego: any,
  ...
  // Retorna el nivel del jugador y el siguiente nivel
  // No entiendo por qué esto es tan lioso. Debe haber alguna forma más sencilla de resolver esto
  ...
  private DameSiguienteNivel(nivelesDelJuego: any, nivelId: number): Nivel {
...
public BorrarPunto(   punto: TablaHistorialPuntosAlumno, alumnoJuegoDePuntos: any, nivel: any,
                      siguienteNivel: any, nivelesDelJuego: any, alumnoSeleccionado: any, juegoSeleccionado: any
  ): any {
  ...
  public DameNivelAnterior(nivelesDelJuego: any, nivelId: number): Nivel {
